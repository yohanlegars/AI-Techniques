...
class ReplayMemory(object):
    ...
    def store_experience(self, prev_obs, action, observation, reward, done):
        ...
        if len(self.prev_observations) > self.size:
    ...
    def sample_batch(self, batch_size):
        ...
        for i in range(batch_size):
            idx[i] = random.randint(0, len(self.prev_observations)-1)

        prev_observations_batch = np.array([self.prev_observations[i] for i in idx])
        actions_batch = np.array([self.actions[i] for i in idx])
        observations_batch = np.array([self.observations[i] for i in idx])
        rewards_batch = np.array([self.rewards[i] for i in idx])
        dones_batch = np.array([self.dones[i] for i in idx])

        return prev_observations_batch, actions_batch, observations_batch, rewards_batch, dones_batch
...
class QLearner(object):
    def __init__(self, env, q_function, target_q_function, discount=DEFAULT_DISCOUNT, rm_size=RMSIZE):
        self.env = env
        self.Q = q_function
        self.target_qn = target_q_function #coding exercise 4
    ...
    def reset_episode(self, initial_obs):
        ...
        self.episode += 1
        self.target_qn.load_state_dict(self.Q.state_dict()) #coding exercise 4
    ...
    def process_experience(self, action, observation, reward, done):
        ...
        self.stage += 1
        self.Q.single_Q_update(prev_observation, action, observation, reward, done, self.target_qn)
        ...
        if self.tot_stages > 10 * self.batch_size:
            prev_observations, actions, observations, rewards, dones = self.rm.sample_batch(self.batch_size)

            self.Q.batch_Q_update(prev_observations, actions, observations, rewards, dones, self.target_qn)
